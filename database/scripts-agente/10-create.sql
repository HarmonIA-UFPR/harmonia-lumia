...
BEGIN 
    -- 1. Tabela HISTORY_CHAT (Esquema: agente)
    CREATE TABLE IF NOT EXISTS agente.history_chat_v2 (
        his_session_uuidv7     uuid NOT NULL,
        his_chat_uuidv7      uuid NOT NULL DEFAULT uuidv7(),
        his_user_uuidv7        uuid NOT NULL,
        his_user_prompt      text,
        his_user_profile     int,
        his_tool_recom_jsonb jsonb, -- Array de objetos JSON
        his_tool_recom_composite_type agente.tool_recommendation[],
        his_llm_response    text,
        his_timestamp       timestamp with time zone DEFAULT now()
    );
...
BEGIN 
    ...
    -- ------------------------------------------------------------------------
    -- Primary Key
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'pk_agente_history_chat_v2') THEN
        ALTER TABLE agente.history_chat_v2 
        ADD CONSTRAINT pk_agente_history_chat_v2 PRIMARY KEY (his_chat_uuidv7);
    END IF;
    ...
    -- Índice GIN para performance no JSONB (Opcional, mas recomendado para arrays)
    CREATE INDEX IF NOT EXISTS idx_history_tools_jsonb 
    ON agente.history_chat_v2  USING GIN (his_tool_recom_jsonb);
